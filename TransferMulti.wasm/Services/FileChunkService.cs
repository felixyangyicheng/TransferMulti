...
public sealed class FileChunkService
{
    ...
    public IEnumerable<(string fileId, string chunkId, long offset, int size)> CreateChunks(string fileId, long fileSize)
    {
        var totalChunks = (int)Math.Ceiling(fileSize / (double)DefaultChunkSize);
        var chunkIndex = 0;
        ...
        while (currentOffset < fileSize && chunkIndex < totalChunks)
        {
            ...
            var chunkSize = (int)Math.Min(DefaultChunkSize, remaining);
